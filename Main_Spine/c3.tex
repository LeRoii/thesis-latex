% !TeX root = ../main.tex
\xchapter{基于双重注意力处理的高效可见光航拍图像小目标检测算法}{Efficient Aerial Object Detection Network Based on Bipartite Attentive Processing}
...
为更进一步验证RNWD的有效性，我们将其与IoU系列之外的其他先进损失函数进行了比较，包括GWD、指数归一化GWD以及KL散度损失。如表\ref{tab7}所示，GWD因其快速增大的梯度而对大误差过于敏感，导致其性能不佳。指数归一化GWD在$AP_{50}$指标上表现出竞争力，但在$AP$上略低于RNWD，与此同时，KL散度损失取得了与RNWD相近的$AP$，却在$AP_{50}$上表现不足。RNWD在两个指标上均提供了最均衡、稳定的性能，验证了其设计的有效性。
